...
- `AGENTS.md` is an optional product surface. Keep this repo's self-hosting setup separate from generic repo defaults.
...
- Keep going until a true blocker needs user input or the user pauses.
...
- If repo `AGENTS.md` is absent, stay quiet unless task needs repo-local Sane setup.
...
- Load `continue`, `sane-outcome-continuation`, `sane-agent-lanes`, and enabled pack skills by trigger.
- Use the repo's own verify commands before claiming success; matching skills own detailed procedure.
