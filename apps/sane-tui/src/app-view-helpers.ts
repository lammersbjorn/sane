...
import { currentAction } from "@sane/sane-tui/shell.js";

import type { ExperienceActionGroup, ExperienceView, SaneTuiAppView } from "./app-view.js";

export type DashboardView = ReturnType<typeof loadDashboardView>;
export type HomeModel = ReturnType<typeof loadHomeScreenFromStatusBundle>;
export type InstallModel = ReturnType<typeof loadAddToCodexScreenFromStatusBundle>;
export type InspectModel = ReturnType<typeof loadStatusScreenFromStatusBundle>;
export type PreferencesModel = ReturnType<typeof loadSettingsScreen>;
export type RepairModel = ReturnType<typeof loadRepairScreenFromStatusBundle>;

export interface AppViewModels {
  home: HomeModel;
...
export function buildExperienceView(
  ...
  switch (dashboard.activeSection.id) {
    case "home":
      return {
        ...
        body: [
          ...
          "Set defaults, refresh Codex add-ons, check health, and get back to work."
        ],
    ...
    case "settings": {
      ...
      return {
        eyebrow: "Work style",
        title: "Configure how Sane guides Codex.",
        body: [
          "Set default models for main sessions, discovery, implementation, review, and fast follow-ups.",
          "Review advanced model details before applying optional Codex settings."
        ],
        primaryActionLabel: cleanActionLabel(recommended?.label ?? dashboard.selectedAction.label),
        primaryActionHint: primaryActionHint(recommended ?? dashboard.selectedAction),
        panels: [
          ...
          },
          {
            ...
            lines: [
              `Guidance options: ${preferences.enabledPacks.join(", ") || "core only"}`,
    ...
    case "status": {
      ...
      return {
        ...
        body: [
          "Status is read-only. It shows the short answer first, then full details when selected.",
...
export function selectedActionHelpLines(
...
export function footerLine(
  chips: DashboardView["chips"],
  mode: SaneTuiAppView["mode"]
): string {
  return `mode ${mode.label.toLowerCase()}  |  ${compactStatusLine(chips)}`;
}

export function footerStatusMap(chips: DashboardView["chips"]): SaneTuiAppView["footer"]["status"] {
  return {
...
export function presentFlag(value: boolean): string {
...
function recommendedAction(dashboard: DashboardView): DashboardAction | null {
...
function homeExperienceTitle(home: HomeModel): string {
  switch (home.recommendedActionId) {
    case "install_runtime":
      return "Give this repo a clean starting point.";
    case "preview_codex_profile":
      return "Review the Codex settings Sane recommends.";
    case "export_all":
      return "Add Sane's Codex add-ons when ready.";
    case "doctor":
      return "Your setup health report is ready.";
    default:
      return "Finish Codex setup before work starts.";
  }
}
...
function selectedActionHelpBuilders(
  ...
  return {
    ...
    show_runtime_summary: (selected) => {
      const model = inspect();
      return detailSelectedActionHelp(selected, [
        "Opens saved `.sane` handoff notes.",
        ...model.runtimeSummary.details
      ]);
    },
    show_config: (selected) => detailSelectedActionHelp(selected, inspect().localConfig.details),
    show_codex_config: (selected) => detailSelectedActionHelp(selected, inspect().codexConfig.details),
    preview_policy: (selected) =>
...
function formatProfileActionHelp(
  action: SelectedAction,
  profile: ProfileActionHelpModel
): string[] {
  return [
    ...selectedActionHelp(action),
    "",
...
function isReadOnlyAction(action: { id: UiCommandId }): boolean {
  return (
...
function compactStatusLine(chips: DashboardView["chips"]): string {
...
function chipValue(chips: DashboardView["chips"], id: string): string {
