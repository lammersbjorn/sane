...
import {
  isManagedBlockedResponseGuardHookCommand,
  isManagedCommandSafetyGuardHookCommand,
  isManagedGeneratedSurfaceGuardHookCommand
...
export function containsManagedCommandSafetyGuardHook(entry: unknown): boolean {
  return hookEntries(entry).some((hook: unknown) => {
    const command = asPlainRecord(hook)?.command;
    return typeof command === "string" && isManagedCommandSafetyGuardHookCommand(command);
  });
}

export function containsManagedGeneratedSurfaceGuardHook(entry: unknown): boolean {
  return hookEntries(entry).some((hook: unknown) => {
    const command = asPlainRecord(hook)?.command;
    return typeof command === "string" && isManagedGeneratedSurfaceGuardHookCommand(command);
  });
}

export function containsManagedBlockedResponseGuardHook(entry: unknown): boolean {
  return hookEntries(entry).some((hook: unknown) => {
    const command = asPlainRecord(hook)?.command;
    return typeof command === "string" && isManagedBlockedResponseGuardHookCommand(command);
  });
}
  ...
  return hookEntries(entry).some((hook: unknown) => {
    ...
    return typeof command === "string"
      && (
        ...
        || isManagedCommandSafetyGuardHookCommand(command)
        || isManagedGeneratedSurfaceGuardHookCommand(command)
        || isManagedBlockedResponseGuardHookCommand(command)
      );
  });
}
