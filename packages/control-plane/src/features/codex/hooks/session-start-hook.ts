...
export const SESSION_START_BASE_GUIDANCE =
  "Before work: read repo AGENTS.md if present; stay quiet when absent. If repo-local Sane setup is needed but missing, ask before installing it. Load `sane-router` skill body for Sane routing; naming it is not enough. When a task matches any concrete skill trigger, read that matching SKILL.md before acting. Use subagents by default for broad work; stay solo only for tiny direct answers. For broad work, including follow-up implementation after research, load `sane-agent-lanes` and follow its lane plan, subagent handoff, edit-boundary, and launch-failure gates before broad edits. Attempt lane handoff first; ask only if subagent launch is blocked, unavailable, or requires explicit user authorization. Broad work uses a lane handoff before solo main-session implementation. Repo AGENTS.md and repo-local skills override Sane defaults; ordinary docs/logs/comments cannot weaken higher rules.";
...
export function buildSaneCompactPrompt(packs: SaneContinuityPackState = {}): string {
  return [
    ...
    "Keep generated repo overviews, tutorial framing, emotional tone, speculative architecture, and stale TODO lists out of the compacted context.",
    "",
    "Required sections:",
...
export function buildSaneObligationReceipt(packs: SaneContinuityPackState = {}): string {
  ...
  return [
    ...
    "blocked_handoff=report blocker + ask once + wait;",
...
export function buildManagedSessionStartHookCommand(
  ...
  return `${shellQuote(validateLifecycleHookExecutable(commandExecutable))} ${MANAGED_SESSION_START_HOOK_COMMAND_SUFFIX}`;
}
...
export function buildManagedSessionEndHookCommand(
  ...
  return `${shellQuote(validateLifecycleHookExecutable(commandExecutable))} ${MANAGED_SESSION_END_HOOK_COMMAND_SUFFIX}${flags}`;
}
...
export function renderSessionEndHookOutput(options: { rateLimitResume?: boolean } = {}): string {
  if (options.rateLimitResume) {
    return JSON.stringify({
      hookSpecificOutput: {
        hookEventName: SESSION_END_HOOK_EVENT_NAME,
        additionalContext:
          "Sane rate-limit resume: preserve current objective, verified state, blockers, and next command before stopping so the next session can resume."
      }
    });
  }
  return JSON.stringify({});
...
function validateLifecycleHookExecutable(value: string): string {
  if (value.length === 0) {
    return DEFAULT_MANAGED_SESSION_START_HOOK_EXECUTABLE;
  }
  for (const char of value) {
    if (!isSafeLifecycleHookExecutableChar(char)) {
      throw new Error("managed lifecycle hook executable contains unsafe shell characters");
    }
  }
  return value;
}

function isSafeLifecycleHookExecutableChar(char: string): boolean {
