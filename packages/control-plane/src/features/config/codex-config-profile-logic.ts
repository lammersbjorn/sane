  ...
  if (context.inventory.status === InventoryStatus.Missing) {
    return {
      ...
      details: [
        ...
        "context7-cli: optional upstream CLI install",
        "grep.app: optional when RTK/local search is unavailable"
      ]
    };
  }
...
function integrationProfileAuditFromConfig(config: TomlTable): IntegrationsProfileAudit {
  ...
  return {
    ...
    recommendedTargets,
    optionalTargets: [
      hasContext7 ? null : "context7-cli",
      hasGrep ? null : "grep.app"
    ].filter((value): value is string => value !== null),
    details: [
      hasPlaywright ? "playwright: keep installed" : "playwright: missing -> recommended",
      hasContext7 ? "context7: existing MCP kept" : "context7-cli: optional upstream CLI install",
      hasGrep ? "grep.app: existing MCP kept" : "grep.app: optional when RTK/local search is unavailable"
    ]
  };
}
