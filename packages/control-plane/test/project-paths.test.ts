...
describe('project path parity', () => {
  ...
  it('falls back from a missing nested start path to the enclosing repo root', () => {
    ...
    const discovered = discoverProjectPaths(join(dir, 'packages', 'control-plane', 'src', 'missing.ts'));
  ...
  it('accepts a file start path', () => {
    const dir = makeTempDir();
    mkdirSync(join(dir, '.git'));
    const file = join(dir, 'packages', 'control-plane', 'src', 'platform.ts');
  ...
  it('keeps repo or workspace roots ahead of nested package.json markers', () => {
    const dir = makeTempDir();
    writeFileSync(join(dir, 'pnpm-workspace.yaml'), 'packages:\n  - "packages/*"\n');
    const packageRoot = join(dir, 'packages', 'control-plane');
    mkdirSync(packageRoot, { recursive: true });
    writeFileSync(join(packageRoot, 'package.json'), '{ "name": "@sane/control-plane" }\n');
