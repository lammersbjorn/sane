...
- warning-only conflict visibility for invalid Codex config and unmanaged `mcp_servers.*` entries, so users can see possible interference without Sane auto-fixing their setup
