...
- warning-only conflict visibility for invalid Codex config, unmanaged `mcp_servers.*`, and enabled `plugins.*` entries, so users can see possible interference without Sane auto-fixing their setup
