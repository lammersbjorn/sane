...
- Canonical status bundles and Inspect overview now surface warning-only Codex config conflict detection for invalid config, unmanaged `mcp_servers.*`, and enabled `plugins.*` entries; this is detect-and-warn only and does not add auto-fix behavior
