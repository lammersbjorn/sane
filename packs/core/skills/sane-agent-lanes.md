...
Turn broad work into small owned lanes that can run in parallel without unclear ownership or hidden background mutation.
...
- the research/product pass is broad enough to need owned parallel lanes
...
- worktree readiness from Inspect when available

## Output

Produce a compact lane plan:
...
2. Inspect current repo state before assigning lanes.
3. Split by ownership, not by vague activity type.
...
7. If higher-priority tool rules require explicit subagent authorization and the user has not provided it, ask for that authorization and pause. Do not continue with a broad solo substitute.
8. If spawn fails or thread cap is hit, close completed agents and retry once with either `message` or `items`, not both.
9. Keep one coordinator lane responsible for integration and review.
10. Stop and ask when write boundaries conflict or the next step needs a human choice.
...
- Do not confuse missing explicit user authorization with a harness block; ask and pause.
